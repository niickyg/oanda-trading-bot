...
def run_optimizer(instrument: str):
    """Invoke optimize.py to generate best_params_{instrument}.json."""
    cmd = [sys.executable, "optimize.py", "--instrument", instrument]
...
def evaluate_strategies(instrument: str, best_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for name, params in best_params.items():
        ...
        raw = name.lower()
        if raw == "macdtrend":
            module_name = "strategy.macd_trends"
        elif raw == "rsireversion":
            module_name = "strategy.rsi_reversion"
        else:

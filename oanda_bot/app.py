...
"""
Streamlit app for the OANDA high-frequency bot.

Guarded so it only executes when run via `streamlit run`.
When imported (e.g., for tests), nothing runs and no Streamlit context is required.
"""


def _running_with_streamlit() -> bool:
    """Return True only when executed under `streamlit run`.

    We check for a live Streamlit ScriptRunContext. If import fails or no
    context is present, assume we're not under Streamlit.
    """
    try:
...
def main() -> None:
    ...
    # Local imports are inside main() to avoid side effects when this module is imported
...
if _running_with_streamlit():
    ...
    # Executed directly without Streamlit – exit with a helpful message
